...
export function setupNetherseaBrand()
{
    ...
    nsBrand.combat.addAction(new Action(
        ...
        draws nourishment from it. Any seaborn gets at least, ${wrapRoll([3, D8])} regeneration / round, +15 ft
        movement and 600 ft tremorsense (only within the brand). More powerful 
        seaborn may gain other abilities.</p>`
    ));
    ...
    nsBrand.combat.addAction(new Action(
        ...
        ${wrapRoll(D12)} necrotic (void) damage and ${wrapRoll(D12)} ${wrapDamageType(DamageType.Neural)} 
