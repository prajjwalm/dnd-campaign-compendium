...
export function setupJordi()
{
    ...
    // Setup D&D stats.
    jordi.dStats.initializeStats(10, 14, 14, 13, 19, 13);
    ...
    jordi.card.addCardTag("CR 5");
    jordi.card.addCardTag("From | Terra (Atlava)");
    jordi.card.addCardTag("Race | Ocean Elf");

    jordi.dSKills.setSkillProficiency(DSkill.Acrobatics, Hidden);
    jordi.dSKills.setSkillProficiency(DSkill.Perception, Hidden);
    jordi.dSKills.setSkillProficiency(DSkill.Insight,    Hidden, ProficiencyLevel.Expert);
    jordi.dSKills.setSkillProficiency(DSkill.Medicine,   Hidden, ProficiencyLevel.Expert);
    jordi.dSKills.setSkillProficiency(DSkill.Religion,   Hidden);
    jordi.dSKills.setSkillProficiency(DSkill.Persuasion, Hidden);

    jordi.operator.morale = Morale.Average;
    jordi.dSKills.finalizeSkills();

    jordi.cSkills.setSkillValues([
    ...
    jordi.operator.fatigue = 0;
    jordi.operator.ratings = {
        damage  : "C",
        control : "C+",
        survival: "B",
        pro     : "B",
    };
    jordi.operator.addNotableStuff("Major Damage Type", "Radiant");
    jordi.operator.addNotableStuff("Strong against", "Seaborn, Status-Inflicting Enemies");
    jordi.operator.addNotableStuff("Weak against", "Martials");
    jordi.operator.addNotableStuff("Combat Experience", "C Grade (1 years)");
    jordi.operator.setChemistryWith(NpcID.Hav, 21, "His GodFather/adoptive uncle, and the only link to his past. Had a great deal of respect for Hav.");
    jordi.operator.setChemistryWith(NpcID.Irene, 17, "Really took to her after she arrived. Perceived a hidden depth of character to her and resonated " +
                                                     "strongly with that. Crushes on her.");
    jordi.operator.setChemistryWith(NpcID.Elysium, 16, "For some reason, Elysium had always been kind to him, and Jordi reciprocates that. Now he barely " +
                                                       "begins to remember that he might've met him before...");
    jordi.operator.setChemistryWith(NpcID.Petra, 14, "She's been a really kind grandma to him since she arrived. Something nags at the back of his mind " +
                                                     "when with her, but he just can't pinpoint it...");
    jordi.operator.setChemistryWith(NpcID.Hina, 11, "Thinks of her as a younger sister, though he's not fully sure why.");
    jordi.operator.setChemistryWith(NpcID.Yuki, 10, "Always took his side against Kastor's bullying and to Jordi he seems to radiate the aura of a " +
                                                    "noble warrior.");
    jordi.operator.professions = ["Church Keeper", "Fisherman"];
    jordi.operator.era = Era.Renaissance;


    jordi.sheet.cr = new CRValue(5);
    jordi.operator.addNotableStuff("Challenge Rating", "5");

    jordi.combat.setSpeed(Speed.Walking,  30);
    jordi.combat.setSpeed(Speed.Swimming, 55);

    jordi.combat.setSave(DStat.Wis);
    jordi.combat.setSave(DStat.Cha);

    jordi.combat.addClassLevels(AdventurerClass.Cleric, 3);
    jordi.combat.addClassLevels(AdventurerClass.Rogue, 2);
    jordi.combat.bonusHP = 9;
    jordi.combat.computeHP();

    jordi.combat.setSense(Sense.Darkvision, 60);

    jordi.combat.setRes(DamageType.Cold, 50);
    jordi.combat.setRes(DamageType.Poison, 50);


    jordi.combat.addAction(new Action(
        Activation.Special,
    ...
    jordi.combat.addAction(new Action(
        Activation.Action,
        `<p><strong><em>Warm Drizzle.</em> (${jordi.WIS} / LR) </strong> For the
        next minute, Jordi can, as an action, heal an ally within 90 ft for 
        ${wrapRoll([[2, D6], [jordi.WIS + 3, D1]])} HP. Anyone healed by Jordi 
        ...
        heal for ${wrapRoll([[1, D12], [jordi.WIS + 3, D1]])} HP.
    ...
    jordi.combat.addAction(new Action(
        Activation.Action,
        `<p><strong><em>Graceful Downpour.</em> (${jordi.Prof} / SR) </strong> 
         Jordi heals three creatures within 120 ft for ${wrapRoll([[3, D6], [jordi.WIS + 8, D1]])} HP
    ...
    jordi.combat.addAction(new Action(
        ...
        `<p><strong><em>The Lantern Undying.</em> (1 / LR) </strong> 
         ...
         ${jordi.WIS + jordi.Prof} legendary actions. He can consume 2 legendary actions
         to heal a creature within 150 ft for ${wrapRoll([[6, D6], [jordi.WIS + jordi.Prof + 18, D1]])} HP
         ...
         again and remove ALL negative status effects they have.</p>`
    ));
    ...
    jordi.combat.addAction(new Action(
        ...
        the creature for ${wrapRoll([[2, D6], [jordi.WIS + 3, D1]])} HP, granting
        them immunity to suffering any new status effects until the end of that turn.</p>`
    ));

    jordi.sheet.size = CreatureSize.Medium;
    jordi.sheet.subtitle = " Humanoid (Sea Elf), Neutral Good";
    jordi.sheet.acDesc = "(Natural Armor)";
    jordi.sheet.category = "human";
}
